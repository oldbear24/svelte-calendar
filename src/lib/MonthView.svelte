<script lang="ts">
	...
	const weeks = $derived.by(() => {
		...
		const endDate = new SvelteDate(lastDay);
...
<div class="flex flex-1 flex-col overflow-hidden">
	...
	<div class="grid grid-cols-7 border-b border-base-300">
		{#each DAYS as day (day)}
			<div
				class="py-2 text-center text-xs font-semibold tracking-wide text-base-content/60 uppercase"
			>
				{day}
			</div>
		{/each}
	...
	>
		{#each weeks as week, i (i)}
			{#each week as day (day.toDateString())}
				{@const dayEvents = getEventsForDay(day)}
				{@const visibleEvents = dayEvents.slice(0, MAX_VISIBLE_EVENTS)}
				{@const overflowCount = dayEvents.length - MAX_VISIBLE_EVENTS}
				<div
					class="flex min-h-20 flex-col border-r border-b border-base-300 p-1 transition-colors hover:bg-base-200/50 {!isCurrentMonth(
						day
					)
						? 'bg-base-200/30'
						: ''}"
